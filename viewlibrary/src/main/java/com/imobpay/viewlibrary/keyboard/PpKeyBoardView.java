...
public class PpKeyBoardView extends KeyboardView {
    ...
    private void drawNumSpecialKey(Key key, Canvas canvas) {
        ...
        // 空白键和 x 不用 btn_keyboard_key2
        if (key.codes[0] == 0){
            drawText(canvas, key);
        }
        
        // -4 为完成 ，当为-4 的时候背景为蓝色
        if (key.codes[0] == -4 && key.label != null)
        {
            drawKeyBackground(R.drawable.btn_keyboard_sure, canvas, key);
            drawText(canvas, key);
        }
    ...
    private void drawABCSpecialKey(Key key, Canvas canvas) {
        ...
        if (key.codes[0] == 123123 || key.codes[0] == 789789) {
            ...
            drawKeyBackground(R.drawable.btn_keyboard_fh_key, canvas, key);
            drawText(canvas, key);
        }
        // -4 为完成 ，当为-4 的时候背景为蓝色
        if (key.codes[0] == -4 && key.label != null)
    ...
    private void drawSymbolSpecialKey(Key key, Canvas canvas) {
        ...
        if (key.codes[0] == 0 || key.codes[0] == 456456) {
    ...
    private void drawText(Canvas canvas, Key key) {
        ...
        if (key.codes[0] == 46) {
            paint.setTextSize(UIViewUtils.getDensity(mContext)+5);
        }else {
            paint.setTextSize(UIViewUtils.getDensity(mContext));
        }
        ...
        if (mKeyBoard.equals(KeyboardUtil.numKeyboard)) {
            if (key.label != null && key.codes[0] != -4) {
            ...
            if (key.codes[0] == -4){  //因为keylabel 不为空所以不能else
            ...
            if (key.codes[0] == -4){  //因为keylabel 不为空所以不能else
          	  paint.setColor(Color.WHITE);
          	  paint.setTextSize(UIViewUtils.getDensity(mContext) - 3);
          	  paint.getTextBounds(key.label.toString(), 0, key.label.toString()
                        .length(), bounds);
                canvas.drawText(key.label.toString(), key.x + (key.width / 2),
                        (key.y + key.height / 2) + bounds.height() / 2, paint);
