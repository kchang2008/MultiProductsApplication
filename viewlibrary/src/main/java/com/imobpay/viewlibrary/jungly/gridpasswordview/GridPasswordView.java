package com.imobpay.viewlibrary.jungly.gridpasswordview;
...
import com.imobpay.viewlibrary.jungly.gridpasswordview.imebugfixer.ImeDelBugFixedEditText;
...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private Boolean IsShowSoft  =false;  //控制是否调用系统键盘
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (textSize != -1) {
            this.textSize = UIViewUtils.px2sp(context, textSize);
        }

        lineWidth = (int) ta.getDimension(R.styleable.gridPasswordView_lineWidth, UIViewUtils.dp2px(getContext(), 1));
    ...
    public Boolean getIsShowSoft() {
		return IsShowSoft;
	}

	public void setIsShowSoft(Boolean isShowSoft) {
		IsShowSoft = isShowSoft;
	}

	public ImeDelBugFixedEditText.OnDelKeyEventListener onDelKeyEventListener = new ImeDelBugFixedEditText.OnDelKeyEventListener() {
    ...
    private TextWatcher textWatcher = new TextWatcher() {
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            ...
            if (newStr.length() == 1) {
                ...
                //LogUtils.showLog("TAG", "输入的是"+newNum);
    ...
    private OnKeyListener onKeyListener = new OnKeyListener() {
        @Override
        public boolean onKey(View v, int keyCode, KeyEvent event) {  //这里本来只有系统键盘回退管用 现在在这里加上 Keyboard.KEYCODE_DELETE 的事件，让点击回退可以有用
            if (event.getAction() == KeyEvent.ACTION_DOWN && event.getKeyCode() == KeyEvent.KEYCODE_DEL||event.getKeyCode()==Keyboard.KEYCODE_DELETE) {
